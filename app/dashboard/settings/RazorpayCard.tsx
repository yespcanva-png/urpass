...
import { savePaymentSettings, removePaymentSettings } from "@/app/actions/payment-settings";
...
export default function RazorpayCard({ canUsePayments, existingKeyId }: Props) {
  ...
  async function handleInstall() {
    setState("installing");
    // Brief loading to indicate "loading integration"
  ...
  async function handleSave() {
    setError("");
    setSaving(true);
    const result = await savePaymentSettings(keyId.trim(), keySecret.trim());
    setSaving(false);
    if (result.error) {
  ...
  async function handleRemove() {
    setRemoving(true);
    setError("");
    const result = await removePaymentSettings();
    setRemoving(false);
    if (result.error) {
      setError(result.error);
    } else {
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm p-5 flex flex-col gap-3">
        ...
        <p className="text-xs text-neutral-500 leading-relaxed">
          Accept payments directly into your Razorpay account when attendees register for paid events.
        </p>
        <Link
          href="/billing"
          className="inline-flex items-center gap-1.5 text-xs font-bold text-white px-3 py-1.5 rounded-lg hover:opacity-90 transition-opacity self-start"
  ...
  if (state === "idle") {
    return (
      <div className="bg-white rounded-2xl shadow-sm p-5 flex flex-col gap-3">
        ...
        <p className="text-xs text-neutral-500 leading-relaxed">
          Accept payments directly into your Razorpay account.
        </p>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="h-1 bg-neutral-100 overflow-hidden">
          <div
            ...
            style={{ background: "#072654", width: "60%" }}
          />
        </div>
      </div>
    );
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            <button
              onClick={() => { setState("idle"); setKeyId(""); setKeySecret(""); setError(""); }}
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm p-5 flex flex-col gap-3">
      ...
      <p className="text-xs text-neutral-400">
        Payments from your paid events go directly to this Razorpay account.
      </p>
