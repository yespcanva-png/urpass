...
import {
  Loader2, AlertCircle, Eye, EyeOff, Trash2, ExternalLink, X,
} from "lucide-react";
import { saveOrgPaymentSettings, removeOrgPaymentSettings } from "@/app/actions/org-payment-settings";

function RazorpayWordmark({ height = 22 }: { height?: number }) {
  const width = Math.round(height * (122.88 / 26.53));
  return (
    <svg width={width} height={height} viewBox="0 0 122.88 26.53" xmlns="http://www.w3.org/2000/svg">
      <g>
...
const inputCls =
  "bg-neutral-50 border border-neutral-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-brand focus:bg-white transition-all w-full placeholder:text-neutral-400 font-mono";

export default function OrgRazorpayCard({ orgId, orgSlug, existingKeyId }: Props) {
  const [state, setState] = useState<CardState>(existingKeyId ? "connected" : "idle");
  ...
  const [connectedKeyId, setConnectedKeyId] = useState(existingKeyId ?? "");
  ...
  async function handleSave() {
    setError("");
    setSaving(true);
    const result = await saveOrgPaymentSettings(orgId, orgSlug, keyId.trim(), keySecret.trim());
    setSaving(false);
    if (result.error) {
      setError(result.error);
    } else {
      setConnectedKeyId(keyId.trim());
      setKeyId("");
      setKeySecret("");
      setState("connected");
    }
  ...
  async function handleRemove() {
    setRemoving(true);
    setError("");
    const result = await removeOrgPaymentSettings(orgId, orgSlug);
    setRemoving(false);
    if (result.error) {
      setError(result.error);
    } else {
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm p-5 flex flex-col gap-3">
        <div className="flex items-center justify-between gap-3">
          <RazorpayWordmark height={20} />
          <button
            onClick={handleInstall}
            className="shrink-0 text-xs font-bold text-white px-4 py-2 rounded-xl hover:opacity-90 transition-opacity"
        ...
        <p className="text-xs text-neutral-500 leading-relaxed">
          Accept payments for all events in this organization. Revenue goes directly to your Razorpay account.
        </p>
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="p-5 flex flex-col gap-3">
          <RazorpayWordmark height={20} />
          <p className="text-xs text-neutral-500">Loading integration…</p>
          <div className="flex items-center gap-2">
            <Loader2 className="w-3.5 h-3.5 animate-spin text-brand" />
            <span className="text-xs text-brand font-medium">Preparing setup…</span>
          </div>
        </div>
        <div className="h-1 bg-neutral-100 overflow-hidden">
          <div className="h-full rounded-full animate-pulse" style={{ background: "#072654", width: "60%" }} />
        </div>
      </div>
    );
  }
  ...
  if (state === "form") {
    return (
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div className="p-5 flex items-center justify-between gap-3 border-b border-neutral-50">
          <div className="flex flex-col gap-1">
            <RazorpayWordmark height={20} />
            <p className="text-xs text-neutral-500">Enter your API credentials to connect</p>
          </div>
          <button
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wide">Key ID</label>
            <input type="text" placeholder="rzp_live_xxxxxxxxxxxxxxxx" value={keyId}
              onChange={(e) => setKeyId(e.target.value)} className={inputCls} autoComplete="off" spellCheck={false} />
          </div>

          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-neutral-500 uppercase tracking-wide">Key Secret</label>
            <div className="relative">
              <input type={showSecret ? "text" : "password"} placeholder="••••••••••••••••••••••••" value={keySecret}
                onChange={(e) => setKeySecret(e.target.value)} className={`${inputCls} pr-10`}
          ...
          <div className="flex items-center gap-2 pt-1">
            <button onClick={handleSave} disabled={saving || !keyId.trim() || !keySecret.trim()}
              className="flex items-center gap-2 text-sm font-bold text-white px-5 py-2.5 rounded-xl hover:opacity-90 transition-opacity disabled:opacity-40"
              style={{ background: "#072654" }}>
              {saving && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
              {saving ? "Connecting…" : "Save & connect"}
            </button>
            <button onClick={() => { setState("idle"); setKeyId(""); setKeySecret(""); setError(""); }}
              className="text-sm font-medium text-neutral-400 hover:text-neutral-600 px-3 py-2.5">
              Cancel
            </button>
          </div>
        </div>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between gap-3">
        <RazorpayWordmark height={20} />
        <span className="flex items-center gap-1.5 text-xs font-semibold text-green-700 bg-green-50 border border-green-100 px-2.5 py-1 rounded-full shrink-0">
          <span className="w-1.5 h-1.5 rounded-full bg-green-500" />
          Connected
        </span>
      </div>
      <p className="text-xs font-mono text-neutral-500 truncate">{connectedKeyId}</p>
      <p className="text-xs text-neutral-400">
        Revenue from all org events goes directly to this Razorpay account.
      </p>
