...
describe("Payment Verification in Subscription Activation", () => {
  ...
  it("switchPlan rejects non-free plans", async () => {
    const { switchPlan } = await import("@/app/actions/billing");
    const result = await switchPlan("enterprise");
    expect(result?.error).toMatch(/paid plans require payment verification/i);
  });
});
